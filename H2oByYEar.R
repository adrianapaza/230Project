...
sum(DTMALLTopthreeRealWords$IT == T)
...
if  (names(train.h2o13_16[,310]) == 'IT')
{y.dep <- 310 #Set the dependent Y variable
x.indep <- c(1:309) #set X inputs to be the word frequences of the 309 words
}else{
  message("Wrong variable, IT not used")
}

#Initalize vectors that will be used to store accuracy.
totalerrors13_16<-vector(,50)
totalerrors09_12<-vector(,50)
totalerrors04_08<-vector(,50)

#set the hidden layers and neurons hyperparameter
HiddenLayers<-c(2,1) # in this case their are two hidden layers, the first has two neurons and second has one
...
Learning04_08.model <- h2o.deeplearning(y = y.dep, #set outcome variable as IT
                                        ...
                                        epoch = 60,#set number of epochs
                                        hidden = HiddenLayers,
                                        activation = "Tanh", #set the activation function. Tanh was chosen after tuning.
                                        l2=2e-3, #choose L2 loss and set parameter for penalizing weights
...
totalerrorsFrom13_13_16<-vector(,50)
totalerrorsFrom13_09_12<-vector(,50)
totalerrorsFrom13_04_08<-vector(,50)
...
#initalize vectors so that we may later compare data
totalerrorsFrom09_13_16<-vector(,50)
totalerrorsFrom09_09_12<-vector(,50)
totalerrorsFrom09_04_08<-vector(,50)
##now get average change in error between test data and other data
for (i in 1:50) {
...
sum(totalerrors13_16)/50
sum(totalerrors09_12)/50
sum(totalerrors04_08)/50

sum(totalerrorsFrom13_13_16)/50
sum(totalerrorsFrom13_09_12)/50
sum(totalerrorsFrom13_04_08)/50

sum(totalerrorsFrom09_13_16)/50
sum(totalerrorsFrom09_09_12)/50
sum(totalerrorsFrom09_04_08)/50
